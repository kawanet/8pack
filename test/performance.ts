...
describe(`REPEAT=${REPEAT} mocha ${TITLE}`, () => {
...

    {
        ...
        const encoded = epack.encode(source);

        it(`encode short OneByteString (${encoded.length})`, test(() => epack.encode(source), encoded));
        it(`decode short OneByteString (${encoded.length})`, test(() => epack.decode(encoded), source));
    }

    {
        ...
        it(`encode long OneByteString (${encoded.length})`, test(() => epack.encode(source), encoded));
        it(`decode long OneByteString (${encoded.length})`, test(() => epack.decode(encoded), source));
    }

    {
        ...
        it(`encode short TwoByteString (${encoded.length})`, test(() => epack.encode(source), encoded));
        it(`decode short TwoByteString (${encoded.length})`, test(() => epack.decode(encoded), source));
    }

    {
        ...
        it(`encode long TwoByteString (${encoded.length})`, test(() => epack.encode(source), encoded));
        it(`decode long TwoByteString (${encoded.length})`, test(() => epack.decode(encoded), source));
    }

    {
        ...
        it(`encode Int32 (${encoded.length})`, test(() => epack.encode(source), encoded));
        it(`decode Int32 (${encoded.length})`, test(() => epack.decode(encoded), source));
    }

    {
        ...
        it(`encode Float64 (${encoded.length})`, test(() => epack.encode(source), encoded));
        it(`decode Float64 (${encoded.length})`, test(() => epack.decode(encoded), source));
    }
});
